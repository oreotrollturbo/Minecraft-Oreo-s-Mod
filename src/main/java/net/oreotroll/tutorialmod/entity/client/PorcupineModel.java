...
import net.oreotroll.tutorialmod.entity.animation.ModModelAnimations;
...
public class PorcupineModel<T extends PorcupineEntity> extends SinglePartEntityModel<T> {
	...
	public void setAngles(PorcupineEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		this.getPart().traverse().forEach(ModelPart::resetTransform);//ALWAYS CALL THIS
		this.setHeadAngles(netHeadYaw,headPitch);

		this.animateMovement(ModModelAnimations.PORCUPINE_WALK, limbSwing, limbSwingAmount, 2f,2.5f);// change to see what happens?
		this.updateAnimation(entity.idleAnimationState, ModModelAnimations.PORCUPINE_IDLE,ageInTicks,1f);
		this.updateAnimation(entity.attackingAnimationState, ModModelAnimations.PORCUPINE_ATTACK,ageInTicks,1f);
	}
